...
# HERE on a fait que les SSQXT manquants maintenants 
...
# Faire à la main PLAINE,EMILIE
# AJOUTER DELALANDRE A LA MAIN AUSSI 
# 14A-003 aucun NGS pas de prlvmnt (non vérifié)
...
all_variants_data <- process_xml_folder("./input/bm/XML/missing_selected_021225/")
...
(
  ...
  # %>% mutate(alternative = ifelse(nom == "DIJON,VALERIE" & gene == "NPM1", "TCTGC", alternative))
  # %>% mutate(reference = ifelse(nom == "DIJON,VALERIE" & gene == "NPM1", "T", reference))
) -> SQXT_ndosbm_row_missing

(
  SQXT_ndosbm_row
  %>% bind_rows(SQXT_ndosbm_row_missing)
) -> SQXT_ndosbm_row_total

length(unique(SQXT_ndosbm_row_total$ndosbm))
...
(
  ...
  %>% ungroup()
  # %>% filter(to_drop == FALSE)
  %>% left_join(SQXT_ndosbm_row_total %>% rename(ndosbm_diag = ndosbm))
  %>% mutate(transcript = "")
  %>% mutate(exon = "")
  %>% left_join(bm_BIG %>% select(ndosbm_diag = n_bm, DDG = ddp))
  %>% left_join(bm_BIG %>% select(ndosbm_suivi = n_bm, date_suivi = ddp))
...
# length(unique(correspondance_diag_suivi$ndosbm))
# length(unique(SQXT_ndosbm_row_total$ndosbm))
length(unique(FC_021225$ndosbm_suivi))
